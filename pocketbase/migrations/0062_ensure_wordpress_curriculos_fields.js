migrate(
  (app) => {
    ...
    const saved = app.findCollectionByNameOrId('vacancies')
    if (
      !saved.fields.getByName('wordpress_curriculos_count') ||
      !saved.fields.getByName('wordpress_curriculos_synced_at')
    ) {
      throw new Error('Falha ao materializar campos de currículos do WordPress')
    }
  },
  (app) => {
    const col = app.findCollectionByNameOrId('vacancies')
