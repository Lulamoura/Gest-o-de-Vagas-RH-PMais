routerAdd(
  ...
  (e) => {
    try {
      ...
      if (userDepto) {
        try {
          var dept = $app.findRecordById('departamentos', userDepto)
      ...
      try {
        ...
        $app.logger().error('requisition_status_change: history failed', 'error', String(histErr))
      }
      ...
      try {
        ...
        for (var m = 0; m < notifyUserIds.length; m++) {
          try {
            ...
            $app
              .logger()
              .error(
                'requisition_status_change: single notification failed',
                'error',
                String(notifItemErr),
              )
          }
        ...
        $app
          .logger()
          .error('requisition_status_change: notification block failed', 'error', String(notifErr))
      }

      return e.json(200, { success: true, oldStatus: oldStatus, newStatus: newStatus })
    } catch (err) {
      $app.logger().error('requisition_status_change: unexpected error', 'error', String(err))
      return e.internalServerError('Erro ao atualizar status da requisição')
    }
  },
  $apis.requireAuth(),
)
